...
{#if $lightboxMedia}
    <div class="fixed inset-0 bg-black/70 backdrop-blur-xs flex items-center justify-center z-50" transition:fade={{ duration: 500 }}>
        <img src="{$lightboxMedia.url}?width=1920&quality=50" alt="{$lightboxMedia.alt}" class="max-h-[92vh] max-w-[95vw] rounded">
    
        <button class="absolute top-2 right-2 bg-white rounded-full hover:scale-105 transition duration-500 cursor-pointer" on:click={() => $lightboxMedia = null} aria-label="close">
